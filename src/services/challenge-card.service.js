...
class ChallengeCardService {
    ...
    async ensureRematch(challenge, winnerUserId) {
        ...
        // A rematch is generated BY the card, not created by a person. Leaving
        // its 'created' event in place inflated the funnel by exactly the
        // number of cards rendered — 10 created = 7 real + 3 rematches — and
        // made it look as though challenges were appearing on their own.
        await pool.query(
            `DELETE FROM challenge_events WHERE challenge_id = $1 AND event = 'created'`,
