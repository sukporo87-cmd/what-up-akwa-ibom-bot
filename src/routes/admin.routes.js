...
router.get('/api/challenges/list', authenticateAdmin, async (req, res) => {
  try {
    ...
    // Rematches are excluded by default: they are generated by a result card,
    // not created by a person, and listing them makes it look as though twice
    // as many challenges exist as anybody started.
    const includeRematches = req.query.rematches === '1';

    const result = await pool.query(`
      SELECT c.id, c.code, c.mode, c.format, c.status, c.categories,
             c.prize_amount, c.entry_model, c.created_at, c.completed_at,
             c.integrity_hold, c.created_platform,
             (c.settings ? 'rematchOf')                                   AS is_rematch,
             ...
             (SELECT u2.username FROM challenge_participants p
                JOIN users u2 ON u2.id = p.user_id
               WHERE p.challenge_id = c.id AND p.rank = 1 LIMIT 1)         AS winner
      FROM challenges c
      JOIN users u ON u.id = c.creator_user_id
      WHERE ($1::boolean OR NOT (c.settings ? 'rematchOf'))
      ORDER BY c.created_at DESC
      LIMIT $2 OFFSET $3
    `, [includeRematches, limit, offset]);

    const total = await pool.query(
      `SELECT COUNT(*)::int AS n FROM challenges
       WHERE ($1::boolean OR NOT (settings ? 'rematchOf'))`,
      [includeRematches]
    );

    res.json({ success: true, total: total.rows[0].n, challenges: result.rows });
  } catch (error) {
...
router.get('/api/challenges/:id/detail', authenticateAdmin, async (req, res) => {
  try {
    ...
    const detail = await challengeService.getAdminDetail(parseInt(req.params.id, 10));
    if (!detail) return res.status(404).json({ success: false, error: 'Not found' });
    res.json({ success: true, ...detail });
  } catch (error) {
    logger.error('Error loading challenge detail:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});
